...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --navy: #1B2B4B; --gold: #C49A45; --cream: #FAF8F3; --warm: #F2EDE3;
      --muted: #6B7B8A; --white: #ffffff; --green: #2D6A4F; --amber: #B7791F; --red: #9B2C2C;
    }
    ...
    .option-btn { display: flex; align-items: center; gap: 14px; background: var(--cream); border: 1.5px solid rgba(27,43,75,0.1); border-radius: 10px; padding: 1rem 1.2rem; cursor: pointer; transition: all 0.2s; text-align: left; font-family: 'DM Sans', sans-serif; font-size: 0.92rem; color: var(--navy); width: 100%; }
    .option-btn:hover { border-color: var(--gold); background: rgba(196,154,69,0.06); transform: translateX(3px); }
    .option-btn.selected { border-color: var(--gold); background: rgba(196,154,69,0.1); }
...
<script>
var questions = [
  { id: 'hours', text: 'How many hours of sleep do you usually get on a weeknight?', sub: 'Think about the last few weeks on average.', options: [
  ...
  { id: 'quality', text: 'How would you rate the quality of your sleep overall?', sub: 'Not just the hours, but how rested you feel.', options: [
  ...
  { id: 'falling', text: 'How long does it typically take you to fall asleep?', sub: 'From the moment you turn the light off.', options: [
  ...
  { id: 'waking', text: 'Do you wake up during the night?', sub: 'Not counting toilet trips for those over 50.', options: [
  ...
  { id: 'position', text: 'What is your primary sleeping position?', sub: 'The position you spend most of the night in.', options: [
  ...
  { id: 'mattress', text: 'How old is your current mattress?', sub: 'Most mattresses should be replaced every 7-10 years.', options: [
  ...
  { id: 'screen', text: 'Do you use screens (phone, TV, laptop) in the hour before bed?', sub: 'Blue light suppresses melatonin production.', options: [
  ...
  { id: 'daytime', text: 'How do you feel during the day?', sub: 'Your daytime energy is a key indicator of sleep quality.', options: [
...
var recs = {
  mattress: { tag: 'Mattresses', title: 'Best Mattresses in Australia 2026', desc: 'Find the right support for your sleep style and budget', url: '/best-mattress-in-australia-2026' },
  pillow: { tag: 'Pillows', title: 'Best Pillows for Your Sleep Position', desc: 'The right pillow makes a bigger difference than you think', url: '/blog/best-contour-pillow-australia-2026' },
  snore: { tag: 'Pillows', title: 'Best Anti-Snore Pillows Australia', desc: 'Specially designed to keep airways open all night', url: '/blog/best-anti-snore-pillow-australia' },
  backpain: { tag: 'Back Pain', title: 'Best Mattress for Back Pain Australia', desc: 'Physio-approved picks for spinal support', url: '/blog/best-mattress-for-back-pain-australia-2026' },
  koala: { tag: 'Review', title: 'Koala Mattress Review 2026', desc: "Australia's most popular mattress - is it worth it?", url: '/blog/koala-mattress-review-2026' },
  weighted: { tag: 'Sleep Science', title: 'The Science of Weighted Blankets', desc: 'Evidence-based guide to better sleep quality', url: '/blog/the-science-of-weighted-blankets' },
  latex: { tag: 'Pillows', title: 'Best Latex Pillow Australia 2026', desc: 'Natural, cool and durable - expert picks', url: '/blog/best-latex-pillow-australia-2026' }
};

var current = 0;
var answers = {};

function getColor(s) { return s >= 75 ? '#2D6A4F' : s >= 50 ? '#B7791F' : '#9B2C2C'; }
function getLabel(s) { return s >= 75 ? 'Great Sleeper' : s >= 55 ? 'Average Sleeper' : s >= 35 ? 'Poor Sleeper' : 'Sleep Deprived'; }

function updateProgress() {
  var pct = Math.round((current / questions.length) * 100);
  document.getElementById('progress-fill').style.width = pct + '%';
  document.getElementById('progress-label').textContent = current < questions.length ? 'Question ' + (current + 1) + ' of ' + questions.length : 'Results ready';
...
function renderQuestion() {
  ...
  html += '<div class="q-number">Question ' + (current + 1) + ' of ' + questions.length + '</div>';
  html += '<div class="q-text">' + q.text + '</div>';
  html += '<div class="q-sub">' + q.sub + '</div>';
  html += '<div class="options">';
  for (var i = 0; i < q.options.length; i++) {
    var opt = q.options[i];
    var cls = sel === i ? ' selected' : '';
    html += '<button class="option-btn' + cls + '" data-index="' + i + '">';
    html += '<span class="option-icon">' + opt.icon + '</span>';
    html += '<span class="option-text"><span class="option-label">' + opt.label + '</span><span class="option-desc">' + opt.desc + '</span></span>';
    html += '</button>';
  }
  html += '</div>';
  html += '<div class="q-nav">';
  html += '<button class="btn-back" id="btn-back"' + (current === 0 ? ' style="visibility:hidden"' : '') + '>Back</button>';
  var isLast = current === questions.length - 1;
  html += '<button class="btn-next" id="btn-next"' + (sel === undefined ? ' disabled' : '') + '>' + (isLast ? 'See My Results' : 'Next') + '</button>';
  html += '</div></div>';
  ...
  document.querySelectorAll('.option-btn').forEach(function(btn) {
    btn.addEventListener('click', function() {
      answers[questions[current].id] = parseInt(this.dataset.index);
  ...
  document.getElementById('btn-next').addEventListener('click', function() {
    if (current < questions.length - 1) { current++; updateProgress(); renderQuestion(); }
    else { updateProgress(); showResults(); }
  });
...
function showResults() {
  ...
  questions.forEach(function(q) {
    max += 10;
    if (answers[q.id] !== undefined) total += q.options[answers[q.id]].score;
  });
  var pct = Math.round((total / max) * 100);
  var color = getColor(pct);
  var label = getLabel(pct);

  var mattress = answers['mattress'];
  var position = answers['position'];
  var daytime = answers['daytime'];
  var waking = answers['waking'];
  var screen = answers['screen'];
  var hours = answers['hours'];

  var goods = [], warns = [], alerts = [];
  if (hours === 2) goods.push('You are hitting the 7-8 hour sweet spot - the single biggest factor in sleep health.');
  if (answers['quality'] === 3) goods.push('Waking up refreshed is a strong sign your sleep cycles are completing properly.');
  if (answers['falling'] === 1) goods.push('Taking 10-20 minutes to fall asleep is the ideal range - not too quick, not too slow.');
  if (waking === 0) goods.push('Sleeping through the night means your sleep architecture is solid.');
  if (screen === 0) goods.push('Avoiding screens before bed is one of the most impactful sleep hygiene habits you can have.');
  if (mattress === 2) warns.push('Your mattress is 7-10 years old. Most mattresses lose 20-30% of support by this age, affecting sleep quality and back health.');
  if (mattress === 3) alerts.push('A mattress over 10 years old is almost certainly contributing to your sleep problems. This is the highest-priority change you can make.');
  ...
  if (screen === 3) warns.push('Using screens in bed every night suppresses melatonin for up to 3 hours, delaying deep sleep.');
  if (waking === 3) warns.push('Waking multiple times per night fragments your sleep cycles, reducing restorative deep sleep and REM sleep.');
  if (hours === 0 || hours === 1) alerts.push('Consistently getting under 6 hours sleep is associated with serious long-term health risks including cardiovascular disease.');

  var recKeys = [];
  if (mattress >= 2) { recKeys.push('mattress'); recKeys.push('backpain'); }
  else { recKeys.push('koala'); recKeys.push('mattress'); }
  ...
  var seen = {};
  recKeys = recKeys.filter(function(k) { if (seen[k]) return false; seen[k] = true; return true; }).slice(0, 4);

  var circ = 2 * Math.PI * 54;
  var offset = circ - (pct / 100) * circ;

  var intro = pct >= 75 ? 'Your sleep habits are strong - some fine-tuning could make good sleep even better.'
    : pct >= 50 ? 'There is meaningful room to improve your sleep. The recommendations below are your best starting points.'
    : 'Your sleep needs attention. The right changes can transform how you feel within weeks.';

  var html = '<div class="results-card">';
  html += '<div class="results-header">';
  html += '<div class="score-ring-wrap">';
  html += '<svg width="130" height="130" viewBox="0 0 130 130"><circle cx="65" cy="65" r="54" fill="none" stroke="#F2EDE3" stroke-width="10"/>';
  html += '<circle cx="65" cy="65" r="54" fill="none" stroke="' + color + '" stroke-width="10" stroke-dasharray="' + circ + '" stroke-dashoffset="' + offset + '" stroke-linecap="round"/></svg>';
  html += '<div class="score-number"><span class="score-num" style="color:' + color + '">' + pct + '</span><span class="score-max">/100</span></div></div>';
  html += '<h2>Your sleep score is ' + pct + '/100</h2><p>' + intro + '</p></div>';

  html += '<div class="result-sections">';
  goods.forEach(function(g) { html += '<div class="result-section good"><div class="rs-header"><span>✓</span><span class="rs-title">What you are doing well</span></div><p class="rs-text">' + g + '</p></div>'; });
  warns.forEach(function(w) { html += '<div class="result-section warn"><div class="rs-header"><span>⚠</span><span class="rs-title">Worth improving</span></div><p class="rs-text">' + w + '</p></div>'; });
  alerts.forEach(function(a) { html += '<div class="result-section alert"><div class="rs-header"><span>!</span><span class="rs-title">Priority attention needed</span></div><p class="rs-text">' + a + '</p></div>'; });
  html += '</div>';

  html += '<div class="recommendations"><div class="rec-title">Your personalised reading list</div><div class="rec-cards">';
  recKeys.forEach(function(k) {
    html += '<div class="rec-card"><a href="' + recs[k].url + '">';
    html += '<div class="rec-tag">' + recs[k].tag + '</div>';
    html += '<h4>' + recs[k].title + '</h4>';
    html += '<p>' + recs[k].desc + '</p>';
    html += '<span class="rec-arrow">Read guide</span></a></div>';
  });
  html += '</div></div>';

  html += '<div class="result-actions">';
  html += '<button class="btn-share" id="btn-share">Copy result to share</button>';
  html += '<button class="btn-retry" id="btn-retry">Retake quiz</button>';
  html += '</div><p class="share-confirm" id="share-confirm">Copied to clipboard!</p></div>';
